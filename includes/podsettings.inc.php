...
if(empty($new_title) && empty($playlist) && $genre == "select genre" && $_FILES["img-file-mod"]["error"] == 4){
    header("Location: ../home.php?view=profile&uid=".$_SESSION['userUid']."&warning=emptyfields");
} else {
    if(!preg_match("/[\/:*?\"<>|]/", $new_title)){
        $check_title = "SELECT podcastTitle FROM podcasts WHERE podcastTitle = ?";
        $stmt_title_check = mysqli_stmt_init($conn);
        if(!mysqli_stmt_prepare($stmt_title_check, $check_title)){
            header("Location: ../home.php?view=profile&uid=".$_SESSION['userUid']."&error=sqlerror");
            echo "<script>changeUrl('Home', '../home.php?view=profile&uid=".$_SESSION['userUid']."&error=sqlerror')</script>";
            exit();
        } else {
            mysqli_stmt_bind_param($stmt_title_check, "s", $new_title);
            mysqli_stmt_execute($stmt_title_check);
            mysqli_stmt_store_result($stmt_title_check);
            if($stmt_title_check->num_rows > 0){
                echo "<script>changeUrl('Home', '../home.php?view=profile&uid=".$_SESSION['userUid']."&error=exists')</script>";
                header("Location: ../home.php?view=profile&uid=".$_SESSION['userUid']."&error=exists");
                exit();
            } else {
                ...
                if(!mysqli_stmt_prepare($stmt_check, $query_check)){
                    ...
                    if($stmt_check->num_rows > 0){
                            ...
                            if ($_FILES["img-file-mod"]["size"] > 5000000000) {
                                ...
                                if(!empty($playlist) && $playlist != str_repeat("_", strlen($playlist))) {
                                    ...
                                    if (!mysqli_stmt_prepare($stmt_playlist, $update_playlist)) {
                                        echo "<script>changeUrl('Home', '../home.php?view=profile&uid=".$_SESSION['userUid']."&error=sqlerror')</script>";
                                        header("Location: ./home?view=profile&uid=".$_SESSION['userUid']."&error=sqlerror");
                                            exit();
                                    } else {
                        ...
                        echo "<script>changeUrl('Home', '../home.php?view=profile&uid=".$_SESSION['userUid']."&error=notfound".$old_title."')</script>";
                        header("Location: ../home.php?view=profile&uid=".$_SESSION['userUid']."&error=notfound".$old_title);
                    }
                }
            }
        }
        ...
        echo "<script>changeUrl('Home', '../home.php?error=invalidtitle')</script>";
