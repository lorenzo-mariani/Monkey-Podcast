    ...
    if (empty($nameChannel) || empty($uidSub)) {
        ...
        exit();
            ...
            if (!mysqli_stmt_prepare($stmt, $sql_insert)) {
                header("Location: ../content/users/".$nameChannel."/".$nameChannel.".php?error=sqlerror_prepare_insert");
                   exit();
            } else {
                mysqli_stmt_bind_param($stmt, "ss", $nameChannel, $uidSub);
                if(!mysqli_stmt_execute($stmt)){
                    header("Location: ../content/users/".$nameChannel."/".$nameChannel.".php?error=sqlerror_execute_insert");
                }

                $sql_update = "UPDATE users SET subs = subs + 1 WHERE uidUsers = ?";
                if (!mysqli_stmt_prepare($stmt, $sql_update)) {
                    header("Location: ../content/users/".$nameChannel."/".$nameChannel.".php?error=sqlerror_prepare_update");
                    exit();
                } else {
                    mysqli_stmt_bind_param($stmt, "s", $nameChannel);
                    if(!mysqli_stmt_execute($stmt)){
                        header("Location: ../content/users/".$nameChannel."/".$nameChannel.".php?error=sqlerror_execute_update");
                    }
                    mysqli_stmt_close($stmt);
                    mysqli_close($conn);
                    header("Location: ../content/users/".$nameChannel."/".$nameChannel.".php");
                }
            }
    }
?>